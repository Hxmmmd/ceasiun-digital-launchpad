...
type QuoteData = {
  services: string[]; serviceSelections: Record<string, { subservices: string[]; discovery: string }>; goals: string[]; situation: string[]; timeline: string; budget: string;
...
function QuoteIntake({ services, initialService, onSubmit }: { services: ReturnType<typeof useCmsServices>; initialService: string; onSubmit: (data: QuoteData) => void }) {
  ...
  const [data, setData] = useState<QuoteData>({ services: initialService ? [initialService] : [], serviceSelections: initialService ? { [initialService]: { subservices: [], discovery: "" } } : {}, goals: [], situation: [], timeline: "", budget: "", projectType: "", platforms: [], notes: "", name: "", company: "", email: "", phone: "", contactMethod: "Email", files: [] });
  ...
  const updateServiceSelections = (service: string, patch: Partial<{ subservices: string[]; discovery: string }>) => update({ serviceSelections: { ...data.serviceSelections, [service]: { subservices: data.serviceSelections[service]?.subservices ?? [], discovery: data.serviceSelections[service]?.discovery ?? "", ...patch } } });
