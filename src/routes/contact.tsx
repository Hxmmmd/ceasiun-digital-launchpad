...
function QuoteIntake({ services, initialService, onSubmit }: { services: ReturnType<typeof useCmsServices>; initialService: string; onSubmit: (data: QuoteData) => void }) {
  ...
  return <div className="quote-intake">
    ...
    <div className="quote-step" key={step}>
      ...
      {step === 6 && <><p className="eyebrow">07 / YOUR DETAILS</p><h2>Who should we speak with about this project?</h2><div className="quote-field-grid"><label>Full name *<Input value={data.name} onChange={(event) => update({ name: event.target.value })} placeholder="Your name" /></label><label>Company / business<Input value={data.company} onChange={(event) => update({ company: event.target.value })} placeholder="Your company" /></label><label>Email address *<Input type="email" value={data.email} onChange={(event) => update({ email: event.target.value })} placeholder="name@company.com" /></label><label>Phone / WhatsApp <small>(optional)</small><Input type="tel" inputMode="tel" value={data.phone} onChange={(event) => update({ phone: formatInternationalPhone(event.target.value) })} placeholder="+92 300 1234567" /></label></div><label>Preferred contact method<select value={data.contactMethod} onChange={(event) => update({ contactMethod: event.target.value })}><option>Email</option><option>WhatsApp</option><option>Phone call</option></select></label></>}
