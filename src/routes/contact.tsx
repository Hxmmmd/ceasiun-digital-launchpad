...
type QuoteData = {
  services: string[]; subservices: string[]; discovery: string; goals: string[]; situation: string[]; timeline: string; budget: string;
...
function QuoteIntake({ services, initialService, onSubmit }: { services: ReturnType<typeof useCmsServices>; initialService: string; onSubmit: (data: QuoteData) => void }) {
  ...
  const [data, setData] = useState<QuoteData>({ services: initialService ? [initialService] : [], subservices: [], discovery: "", goals: [], situation: [], timeline: "", budget: "", projectType: "", platforms: [], notes: "", name: "", company: "", email: "", phone: "", contactMethod: "Email", files: [] });
  ...
  const toggle = (key: "services" | "goals" | "situation" | "platforms" | "subservices", value: string) => { setErrors([]); update({ [key]: data[key].includes(value) ? data[key].filter((item) => item !== value) : [...data[key], value] }); };
  const selected = data.services.flatMap((service) => serviceQuestions[service] ? [serviceQuestions[service]] : []).reduce((acc, item) => ({ subservices: [...new Set([...acc.subservices, ...item.subservices])], goals: [...new Set([...acc.goals, ...item.goals])], context: [...new Set([...acc.context, ...item.context])], scope: [...new Set([...acc.scope, ...item.scope])] }), { subservices: [], goals: [], context: [], scope: [] } as { subservices: string[]; goals: string[]; context: string[]; scope: string[] });
  const questions = selected.goals.length ? selected : { subservices: [], ...fallbackQuestions };
  const validate = () => { const nextErrors: string[] = []; if (step === 0 && !data.services.length) nextErrors.push("Please select at least one service to continue."); if (step === 1 && (!data.subservices.length || (data.subservices.some((item) => item.includes("not sure")) && data.discovery.trim().length < 10))) nextErrors.push("Choose a capability, or tell us what you are trying to accomplish."); if (step === 2 && !data.goals.length) nextErrors.push("Please select at least one relevant goal."); if (step === 3 && !data.situation.length) nextErrors.push("Please select at least one context option."); if (step === 4 && !data.platforms.length) nextErrors.push("Please select at least one type of work."); if (step === 4 && data.notes.trim().length < 10) nextErrors.push("Please tell us a little about your project before continuing."); if (step === 5 && !data.timeline) nextErrors.push("Please select a preferred timeline."); if (step === 6) { if (!data.name.trim()) nextErrors.push("Please enter your full name."); if (!/^[^\\s@]+@[^\\s@]+\\.[^\\s@]{2,}$/.test(data.email)) nextErrors.push("Invalid email address. Please enter a valid email."); if (data.phone && !/^\\+?[0-9][0-9\\s().-]{6,}$/.test(data.phone)) nextErrors.push("Please enter a valid international phone number."); } setErrors(nextErrors); return nextErrors.length === 0; };
  ...
  return <div className="quote-intake">
    ...
    <div className="quote-step" key={step}>
      ...
      {step === 1 && <><p className="eyebrow">02 / YOUR DIRECTION</p><h2>Which part of {data.services.join(" and ")} do you need?</h2><p className="quote-helper">Choose the capabilities that best describe the project. If you&apos;re not sure, select that and we&apos;ll guide you.</p><div className="quote-options"><div className="quote-chip-grid">{questions.subservices.map((item) => <button type="button" className={data.subservices.includes(item) ? "selected" : ""} onClick={() => { if (item.includes("not sure")) update({ subservices: [item], discovery: "" }); else update({ subservices: data.subservices.includes(item) ? data.subservices.filter((value) => value !== item) : [...data.subservices.filter((value) => !value.includes("not sure")), item] }); }} key={item}>{item}{data.subservices.includes(item) && <Check />}</button>)}</div></div>{data.subservices.some((item) => item.includes("not sure")) && <Textarea value={data.discovery} onChange={(event) => update({ discovery: event.target.value })} rows={5} placeholder="Tell us what you are trying to accomplish and the biggest problem you want to solve..." />}</>}
      {step === 2 && <><p className="eyebrow">03 / DIRECTION</p><h2>What are you looking to achieve?</h2><p className="quote-helper">Questions tailored to {data.services.join(", ")}.</p><div className="quote-options"><div className="quote-chip-grid">{questions.goals.map((item) => <button type="button" className={data.goals.includes(item) ? "selected" : ""} onClick={() => toggle("goals", item)} key={item}>{item}{data.goals.includes(item) && <Check />}</button>)}</div></div></>}
